...
for folder_ind = 1:length(log_folders)
    ...
    if ( length(parse_details) == 4 )
        ...
        errors = load( [experiment1.base_dir '/' log_folders(folder_ind).name '/error.txt'] );
...
for t_ind = 1:length(experiment1.time)
    ...
    title( sprintf( 'Error after %.2f seconds', experiment1.time(t_ind) ) );

    %%
    output_name = ['output_images/' experiment '/weighted_pseudoinverse_error_over_time_video/' sprintf( 'time_%05.2f.eps', experiment1.time(t_ind) )];
